...
function OrderSuccess() {
  ...
  return (
    <div style={{ padding: 40, textAlign: "center", fontFamily: "sans-serif" }}>
      <h1 style={{ color: "green" }}>✅ Payment Successful!</h1>
      <p>
        Order ID: <strong>{p.get("orderId") || "N/A"}</strong>
      </p>
      <p>
        Amount: <strong>PKR {p.get("amount") || "N/A"}</strong>
      </p>
      <a href="/">← Go back</a>
    </div>
  );
...
function OrderFailed() {
  ...
  return (
    <div style={{ padding: 40, textAlign: "center", fontFamily: "sans-serif" }}>
      ...
      <p>
        Error:{" "}
        <strong>{p.get("err_msg") || p.get("code") || "Unknown error"}</strong>
      </p>
      <a href="/">← Try again</a>
    </div>
  );
...
export default function App() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Build hidden form and POST to PayFast
      const form = document.createElement("form");
      form.method = "POST";
